...
// Hermetic cwd: keeps this workspace's real src/lib/og/site.json and
// public/og.jpg (the ASH COW brand) from bleeding into fixture-driven tests.
const CLEAN_CWD = mkdtempSync(join(tmpdir(), "grok-clean-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: CLEAN_CWD });
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", cwd: CLEAN_CWD });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: CLEAN_CWD });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: CLEAN_CWD });
